...
class RequestBase(object):
    ...
    def specification_yaml(self, case_info):
        ...
        try:
            ...
            base_url = self.conf.get_section_for_data('api_envi', 'host')
            yaml_url = case_info["baseInfo"]["url"]

            # --- 智能路由判断逻辑 ---
            if str(yaml_url).startswith('http'):
                # 如果 yaml 中的 url 已经自带了 http/https，说明是完整的绝对路径，直接使用
                url = yaml_url
            else:
                # 如果没有，说明是相对路径（如 /coupApply/cms/...），则拼接环境 host
                url = base_url + yaml_url
            # ------------------------
