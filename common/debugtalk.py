...
class DebugTalk:
    ...
    def get_extract_data(self, node_name, randoms=None) -> str:
        ...
        :return:
        """
        data = GlobalContext.get_var(node_name)
        if data is None:
        ...
        if randoms is not None and bool(re.compile(r'^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$').match(randoms)):
            ...
            context_data = GlobalContext.get_var(node_name, randoms)
            data = context_data if context_data is not None else self.read.get_extract_yaml(node_name, randoms)
        return data
