...
class Assertions:
    ...
    def equal_assert(self, expected_results, actual_results, status_code=None):
        """
        相等断言模式（增强版：支持列表响应与状态码校验）
        """
        flag = 0
        # 修改判断条件：允许 actual_results 为 dict 或 list
        if isinstance(expected_results, dict) and isinstance(actual_results, (dict, list)):
            for assert_key, assert_value in expected_results.items():
                # --- 状态码判断逻辑保持不变 ---
                if assert_key == "status_code":
                    if assert_value != status_code:
                        flag += 1
                        logs.error(f"相等断言失败：预期状态码【{assert_value}】，实际状态码【{status_code}】")
                    else:
                        logs.info(f"状态码相等断言成功：{status_code}")
                    continue

                # --- 增强业务字段判断逻辑 ---
                # 如果响应是列表，先尝试转成字典（如果业务允许）或者在报错时给出友好提示
                if isinstance(actual_results, list):
                    flag += 1
                    logs.error(f"相等断言失败：接口返回的是列表，无法通过 Key【{assert_key}】进行字典断言")
                    continue

                if assert_key in actual_results:
                    actual_value = actual_results[assert_key]
                    if actual_value == assert_value:
                        logs.info(f"相等断言成功：字段【{assert_key}】值【{actual_value}】匹配")
                    else:
                        flag += 1
                        logs.error(f"相等断言失败：字段【{assert_key}】预期【{assert_value}】，实际【{actual_value}】")
                else:
                    flag += 1
                    logs.error(f"相等断言失败：在响应结果中找不到字段【{assert_key}】")
        else:
            raise TypeError('相等断言类型错误，必须为字典类型！')
        return flag
